...
#define C6X36_CHANNEL_NUM	0x0A	//1 Byte	(RW)	TP_Channel_Num		VA channel of number, max 63
#define C6X36_KEY_NUM		0x0B	//1 Byte	读写(RW)	TP_Key_Num		Virtualkey independent channel of number, max 63

#define C6X36_CB_ADDR_W 0x32/*0x32*/	//1 Byte	(RW)	CB_addr		normal mode CB -- addr		
#define C6X36_CB_ADDR_R 0x33/*0x32*/	//1 Byte	(RW)	CB_addr		normal mode CB -- addr			
#define C6X36_CB_BUF  0x39/*0x33*/	//1 Byte	只读(RO)	CB_buf		1 channel mapping 2 char, so len is 2*N						
#define C6X36_RAWDATA_ADDR	0x34/*0x34*/	//1 Byte	(RW)	RawData_addr		Rawdata -- addr			
#define C6X36_RAWDATA_BUF	0x35

#define C6206_FACTORY_TEST_MODE			0xAE	//0:normal mode, 1:factory mode 1 (single+0 level waterproof scan),2:factory mode 2(single+no waterproof scan)
#define C6206_FACTORY_TEST_STATUS		0xAD

#define MAX_SCAP_CHANNEL_NUM		16//Single Chip 72; Double Chips 144
...
//static unsigned char m_bV3TP = false;
...
{
	unsigned char bTestResult = true;
	unsigned char bTempResult = 1;
...
{	
	...
	//enter factory mode successful then load channel of number
...
{	
	...
	for(i = 1; i <= iRedo; i++)
	{
		...
		if(ERROR_CODE_OK != ReCode)
		{
			printk("Failed to Enter work mode...\n");
			if(i < iRedo)
			{
				SysDelay(50);
				continue;
...
static int StartScan(void)
{
	unsigned char RegVal = 0x01;
    	...
    	const unsigned int MaxTimes = 500/*20*/;	//max wiat 160ms
    	unsigned char ReCode = ERROR_CODE_COMM_ERROR;      	
    
    	ReCode = ReadReg(C6208_SCAN_ADDR, &RegVal);
    	if(ReCode == ERROR_CODE_OK)
    	{
    		RegVal = 0x01;		//highest set 1 then start scan
    		ReCode = WriteReg(C6208_SCAN_ADDR, RegVal);
    		if(ReCode == ERROR_CODE_OK)
    		{
    			while(times++ < MaxTimes)		//wait scan successful
    			{
    				SysDelay(8);	//8ms
    				ReCode = ReadReg(C6208_SCAN_ADDR, &RegVal);
    				if(ReCode == ERROR_CODE_OK)
...
unsigned char ReadRawData(unsigned char Freq, unsigned char LineNum, int ByteNum, int *pRevBuffer)
{
    ...
    do{
        WriteReg(C6208_SCAN_ADDR, 0x01);
        msleep(50);
        /* need to get 0x00 on register 0x08
                ready for getting rawdata */
        ReadReg( C6208_SCAN_ADDR, &I2C_wBuffer[0]);
        printk("%s[FTS] C6208_SCAN_ADDR = 0x%02x, I2C_wBuffer[0] = 0x%02x\n", __func__, C6208_SCAN_ADDR, I2C_wBuffer[0]);
        try_cnt++;
    } while ((I2C_wBuffer[0] != 0) && (try_cnt < 10));
	...
	if(ReCode == ERROR_CODE_OK)
	{
		for(i=0; i<(ByteNum>>1); i++)
		{
            ...
            printk("%s [focal] m_ucTempData[%d]=%d, m_ucTempData[%d+1]=%d\n", __func__, i, m_ucTempData[i<<1]<<8, i, m_ucTempData[(i<<1)+1]);
            //printk("%s [focal] pRevBuffer[%d] = %d\n", __func__, i, pRevBuffer[i]);
		}
	}
...
static unsigned char GetChannelNum(void)
{
	...
	if(ReCode == ERROR_CODE_OK)
	{
		g_ScreenSetParam.iChannelsNum = 0;//rBuffer[0];	
	...
	if(ReCode == ERROR_CODE_OK)
	{
		g_ScreenSetParam.iKeyNum= 3;//rBuffer[0];
...
{	
	int i = 0;
	//unsigned char bFlag = true;
	unsigned char ReCode = ERROR_CODE_OK;
	unsigned char btmpresult = true;
	...
	if(ReCode == ERROR_CODE_OK) //load RawData and calculate Differ value
	{
		printk("\r\n//======= Test Data:  ");
		ShowRawData();
	}
	else
	{
		...
		btmpresult = false;//can not get RadData -- NG
		goto TEST_END;
	}

	//----------------------------------------------------------check overflow RawData
	iNgNum = 0;
	iMax = m_RawData[0];
	iMin = m_RawData[0];
	iAvg = 0;
	...
	{
		RawDataMin = g_stCfg_SCap_DetailThreshold.RawDataTest_Min[i];//get detail threshold
		RawDataMax = g_stCfg_SCap_DetailThreshold.RawDataTest_Max[i];//get detail threshold
	...
	////>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>collection test data and store csv file
...
unsigned char FT6X36_TestItem_CbTest(unsigned char * bTestResult)
{
	...
	else
	{
		...
		{			
			...
			/*for(int j = 0; j < 2; j++)//if get data is zero then restart
			{
				if(m_TempCbData[i] == 0)
				{
					SysDelay(20);
	...
	else
	{
		...
		for(i = 0; i < g_ScreenSetParam.iChannelsNum + g_ScreenSetParam.iKeyNum; i++)
		{
			...
			iCbMin = g_stCfg_SCap_DetailThreshold.CbTest_Min[i];//get detail threshold
			iCbMax = g_stCfg_SCap_DetailThreshold.CbTest_Max[i];//get detail threshold
	...
	////<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<collection test data and store csv file
	//GetCiDataMsg();//collection Ci Data and store csv file
...
unsigned char FT6X36_TestItem_DeltaCbTest(unsigned char * bTestResult)
{
	...
	////////////max Delta_Ci and min Delta_Ci differece < default value
	...
	unsigned char bUseCriticalValue = g_stCfg_FT6X36_BasicThreshold.DeltaCbTest_Set_Critical;

	unsigned char bCriticalResult = false;
	...
	unsigned char bUseSort1 = false;
	unsigned char bUseSort2 = false;
	unsigned char bUseSort3 = false;
	unsigned char bUseSort4 = false;
	unsigned char bUseSort5 = false;
	unsigned char bUseSort6 = false;
	...
	//GetDeltaCiDataMsg();//collection Delta Ci Data and store csv file
	return 0;
}
...
unsigned char FT6X36_TestItem_ChannelsDeviationTest(unsigned char * bTestResult)
{
	...
	unsigned char btmpresult = true;//Init
	int i=0;
	...
	unsigned char bUseCriticalValue = g_stCfg_FT6X36_BasicThreshold.ChannelsDeviationTest_Set_Critical;
	...
	int GetDeviation;
	unsigned char bFirstUseSort1 = true;
	unsigned char bFirstUseSort2 = true;
	unsigned char bFirstUseSort3 = true;
	unsigned char bFirstUseSort4 = true;
	unsigned char bFirstUseSort5 = true;
	unsigned char bFirstUseSort6 = true;
	...
	for(i=0; i < g_ScreenSetParam.iChannelsNum/*readlen*/; i++)
	{
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 1)
		{
			...
			{
				if(Sort1LastNum + 1 == i)//nearby channel
				{
					//if((Sort1LastNum == g_ScreenSetParam.iChannelsNum/2 - 1) && (i == g_ScreenSetParam.iChannelsNum/2))
					//{//Main channel separate two part, head part do not compare end part, channel from zero start
                       // head part EOF is g_ScreenSetParam.iChannelsNum/2 - 1
					//}
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 2)
		{
			...
			{
				if(Sort2LastNum + 1 == i)//nearby channel
				{
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 3)
		{
			...
			{
				if(Sort3LastNum + 1 == i)//nearby channel
				{
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 4)
		{
			...
			{
				if(Sort4LastNum + 1 == i)//nearby channel
				{
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 5)
		{
			...
			{
				if(Sort5LastNum + 1 == i)//nearby channel
				{
		...
		if(g_stCfg_SCap_DetailThreshold.DeltaCxTest_Sort[i] == 6)
		{
			...
			{
				if(Sort6LastNum + 1 == i)//nearby channel
				{
...
{	
	...
	unsigned char bUseCriticalValue = g_stCfg_FT6X36_BasicThreshold.TwoSidesDeviationTest_Set_Critical;
