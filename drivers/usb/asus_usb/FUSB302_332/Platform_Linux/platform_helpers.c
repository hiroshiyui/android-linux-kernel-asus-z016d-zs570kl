...
#include "../core/TypeC.h"							// State Machine access
...
FSC_S32 fusb_InitializeGPIO(void)
{
    ...
    if (ret < 0)
    {
        chip->asus_gpios_are_valid = FALSE; // Keep track that our GPIOs are invalid for cleanup
        USB_FUSB302_331_INFO("%s --- GPIO Request Fail Error: %d\n", __func__, ret);
        return ret; // Remove this to avoid failing the probe if the gpio array is invalid, but then make sure you don't try to use these GPIOs later.
    }
    else
    {
        chip->asus_gpios_are_valid = TRUE; // Keep track that our GPIOs are valid for cleanup
        USB_FUSB302_331_INFO("%s --- Asus GPIO array request success!\n", __func__);
    }
    /* ASUS_BSP : Request MUX/Re-Driver GPIO --- */
...
*/
    ...
    if (gpio_is_valid(chip->gpio_VBus5V)) // <-- ASUS_BSP add this line : because we don't use vbus
    {
...
extern FSC_U8 core_get_cc_orientation(void);
void set_asus_mux(void)
{

    struct fusb30x_chip* chip = fusb30x_GetChip();
    FSC_U8 cc = core_get_cc_orientation();
    if (!chip)
    {
        pr_err("FUSB  %s - Error: Chip structure is NULL!\n", __func__);
        return;
    ...
    if (cc != chip->prev_orientation && gpio_is_valid(USB_MUX_GPIO))
    {
        ...
        switch (cc) // CC pin is CC1
        {
            case 0: // No CC pin connected
                //if (gpio_cansleep(USB_RE_GPIO))
                //{
                //    gpio_set_value_cansleep(USB_RE_GPIO, USB_RE_LOW);
                //}
                //else
                //{
                //    gpio_set_value(USB_RE_GPIO, USB_RE_LOW);
                //}
                SinkRequestMaxVoltage = 100;                    // 100 * 50mV  = 5V
                SinkRequestMaxPower   = 10000;                  // 10000 * 0.5mW = 5W
                SinkRequestOpPower = 10000;                     // Operating power the sink will request (10000 * 0.5mW = 5W, used to calculate current as well)
                USB_FUSB302_331_INFO("[PH][MUX]GPIO %d state = %d , GPIO %d state = %d , Pin CC1 state = %d , Pin CC2 state = %d, SinkRequestMaxVoltage = %dV, SinkRequestMaxPower = %dW, SinkRequestOpPower = %dW\n", USB_MUX_GPIO, gpio_get_value(USB_MUX_GPIO),USB_RE_GPIO,gpio_get_value(USB_RE_GPIO), 0, 0,SinkRequestMaxVoltage/20,SinkRequestMaxPower/2000,SinkRequestOpPower/2000);
                break;
            case 1: // CC pin is CC1
            	//if (gpio_cansleep(USB_RE_GPIO))
                //{
                //    gpio_set_value_cansleep(USB_RE_GPIO, USB_RE_HIGH);
                //}
                //else
                //{
            ...
            default: // Unknown/unhandled value
                USB_FUSB302_331_INFO("[PH][MUX]GPIO %d state = %d , GPIO %d state = %d , CC pin orientation detection error! Orientation returned: %d\n", USB_MUX_GPIO, gpio_get_value(USB_MUX_GPIO),USB_RE_GPIO,gpio_get_value(USB_RE_GPIO), cc);
                break;
        }
    }
	USB_FUSB302_331_INFO("[PH][After_MUX]GPIO %d state = %d , GPIO %d state = %d , Pin CC1 state = %d , Pin CC2 state = %d\n", USB_MUX_GPIO, gpio_get_value(USB_MUX_GPIO),USB_RE_GPIO,gpio_get_value(USB_RE_GPIO), blnCCPinIsCC1, blnCCPinIsCC2);
}
...
void fusb_GPIO_Cleanup(void)
{
    ...
    /* Cleanup ASUS GPIOs */
    if (chip->asus_gpios_are_valid == TRUE)
    {
        gpio_free_array(fusb302_gpios, USB_ASUS_NUM_GPIOS);
    }
    /* Cleanup ASUS GPIOs */

    wake_lock_destroy(&chip->fusb302_wakelock);                             // wake lock protection
}
...
// This will handle the "read" file operation for your new sysfs file
static ssize_t _fusb_Sysfs_TypeC_Status_show(struct device* dev, struct device_attribute* attr, char* buf)
{
...
void fusb_Sysfs_Init(void)
{
    FSC_S32 ret = 0;
    struct fusb30x_chip* chip = fusb30x_GetChip();
	USB_FUSB302_331_INFO("fusb_Sysfs_Init start!\n");
    if (chip == NULL)
    {
...
FSC_S32 fusb_EnableInterrupts(void)
{
    ...
    USB_FUSB302_331_INFO("%s - Success: Requested INT_N IRQ: '%d'\n", __func__, chip->gpio_IntN_irq);
    ...
    wake_lock_init(&chip->fusb302_wakelock, WAKE_LOCK_SUSPEND, "fusb302wakelock");  // wake lock protection
    enable_irq_wake(chip->gpio_IntN_irq);                                           // wake lock protection
...
//    } while ((platform_get_device_irq_state()) && (!g_Force_Exit));
    ...
    wake_unlock(&chip->fusb302_wakelock);                               // wake lock protection
    return IRQ_HANDLED;
}
