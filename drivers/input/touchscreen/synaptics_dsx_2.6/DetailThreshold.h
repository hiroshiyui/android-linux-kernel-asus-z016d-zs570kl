...
{   //Start mutual-capacitance
	unsigned char InvalidNode[TX_NUM_MAX][RX_NUM_MAX];//invalid pointer, do not test node
	unsigned char InvalidNode_SC[TX_NUM_MAX][RX_NUM_MAX];//invalid pointer, do not use test node SCAP
	...
	int SCapRawDataTest_ON_Max[TX_NUM_MAX][RX_NUM_MAX];//Start self-capacitance test
	...
	/*int ForceTouch_SCapRawDataTest_ON_Max[FORCETOUCH_ROW][RX_NUM_MAX];//Start force touch test
...
struct stCfg_SCap_DetailThreshold
{
	...
	int DeltaCxTest_Sort[MAX_CHANNEL_NUM];         //6x06 and 6x36 use same array
	int DeltaCxTest_Area[MAX_CHANNEL_NUM];         //6x06 and 6x36 use same array
