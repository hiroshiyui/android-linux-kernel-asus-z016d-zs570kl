...
void InitializePDPolicyVariables(void)
{
    ...
    SinkRequestMaxPower = 10000;                                                // Maximum power the sink will request (5W, used to calculate current as well)
    SinkRequestOpPower = 10000;                                                 // Operating power the sink will request (5W, sed to calculate current as well)
    ...
    CapsSink[0].FPDOSink.OperationalCurrent = 100;                              // Set that our device will consume 1000mA for this object
    ...
    CapsSink[1].FPDOSink.OperationalCurrent = 100;                              // Set that our device will consume 1000mA for this object
...
void PolicySourceSendCaps(void)
{
	...
	{
	    //USB_FUSB302_331_INFO("%s +++ PolicyHasContract is TRUE \n", __func__);
        ...
        else                                                                    // Otherwise...
	{
	    ...
	    //USB_FUSB302_331_INFO("%s +++ PolicyHasContract is FALSE \n", __func__);
	}
    }
    else                                                                        // If we haven't timed out and maxed out on hard resets...
    {
        switch (PolicySubIndex)
        {
			...
			//USB_FUSB302_331_INFO("%s +++ PolicyState = peSourceNegotiateCap (%d)\n", __func__, PolicyState);
		    }
                    else                                                        // Otherwise it was a message that we didn't expect, so...
		    {
                        PolicyState = peSourceSendSoftReset;                    // Go onto issuing a soft reset
			//USB_FUSB302_331_INFO("%s +++ PolicyState = peDisabled (%d)\n", __func__, PolicyState);
		    }
                    PolicySubIndex = 0;                                         // Reset the sub index
                    PDTxStatus = txIdle;                                        // Clear the transmitter status
                }
...
//            else
//            {
//                PolicyState = peDisabled;                    // Go onto issuing a soft reset
////USB_FUSB302_331_INFO("%s +++ PolicyState = peDisabled (%d)\n", __func__, PolicyState);
//            }
            break;
        case 3:
...
void PolicySourceReady(void)
{
    ...
    //USB_FUSB302_331_INFO("PD Policy Source Ready!\n");
}
