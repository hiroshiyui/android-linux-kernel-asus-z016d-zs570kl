...
static void *dhd_wlan_mem_prealloc(int section, unsigned long size)
{
	...
	if (section == DHD_PREALLOC_IF_FLOW_LKUP)  {
		if (size > DHD_PREALLOC_IF_FLOW_LKUP_SIZE) {
			pr_err("request DHD_IF_FLOW_LKUP size(%lu)"
				"is bigger than static size(%d).\n",
...
#ifdef CUSTOMER_HW_ZEN
	.sta_dfs_table = bcm4359_pcie_translate_sta_ap_dfs_table,
	.sta_dfs_size = ARRAY_SIZE(bcm4359_pcie_translate_sta_ap_dfs_table),
	.sta_nodfs_table = bcm4359_pcie_translate_sta_ap_dfs_table,
	.sta_nodfs_size = ARRAY_SIZE(bcm4359_pcie_translate_sta_ap_dfs_table),
	.ap_table = bcm4359_pcie_translate_sta_ap_dfs_table,
	.ap_size = ARRAY_SIZE(bcm4359_pcie_translate_sta_ap_dfs_table),
#else
