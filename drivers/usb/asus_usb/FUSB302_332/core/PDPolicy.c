...
{    
...
#ifdef FSC_HAVE_SNK
    SinkRequestMaxVoltage = 100;                                                // Maximum voltage that the sink will request (180 * 50mV = 9V)
    SinkRequestMaxPower = 10000;                                                // Maximum power the sink will request (10000 * 0.5mW = 5W, used to calculate current as well)
    ...
    CapsSink[0].FPDOSink.Voltage = 100;                                         // Set 5V (100 * 50mV) for the first supply option
    CapsSink[0].FPDOSink.OperationalCurrent = 100;                              // Set that our device will consume 1000mA (100 * 10mA) for this object
    ...
    CapsSink[1].FPDOSink.Voltage = 100;                                         // Set 9V (180 * 50mV) for the second supply option
    CapsSink[1].FPDOSink.OperationalCurrent = 100;                              // Set that our device will consume 1000mA (100 * 10mA) for this object
    ...
    CapsSource[0].FPDOSupply.Voltage = 100;                                     // Set 5V (100 * 50mV) for the first supply option
    CapsSource[0].FPDOSupply.MaxCurrent = 90;                                   // Set 900mA (90 * 10mA) for the first supply option
    CapsSource[0].FPDOSupply.PeakCurrent = 0;                                   // Set peak equal to max
    CapsSource[0].FPDOSupply.DataRoleSwap = FALSE;                              // By default, don't enable DR_SWAP
    ...
    CapsSource[0].FPDOSupply.USBSuspendSupport = FALSE;                         // By default, do not allow USB Suspend
...
//        int PDSinkCaps[14] = {0};
//        PDSinkCaps[0] = 140;	// 7V
//        PDSinkCaps[1] = 80;	// 800mA
//        platform_fusb302_update_sink_capabilities(PDSinkCaps);
//        blnASUStest = FALSE;
//    }
    else
    {
