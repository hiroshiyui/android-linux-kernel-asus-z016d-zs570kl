...
void InitializeTypeCVariables(USBTypeCPort prefer_portype)
{
    ...
    Registers.OCPreg.byte = 0x00;                                               // Set OCPreg to 10mA 0x0000_0000
    DeviceWrite(regOCPreg, 1, &Registers.OCPreg.byte);
...
CCTermType DecodeCCTerminationSource(void)
{
 regMeasure_t saved_measure;   
    ...
    DeviceRead(regStatus0, 1, &Registers.Status.byte[4]);
    if(ConnState == IllegalCable)
#ifdef FSC_DEBUG
    WriteStateLog(&TypeCStateLog, 200, Registers.Measure.MDAC, Registers.Status.COMP);
#endif // FSC_DEBUG
    ...
    if(ConnState == IllegalCable)
#ifdef FSC_DEBUG
    WriteStateLog(&TypeCStateLog, 201, Registers.Measure.MDAC, Registers.Status.COMP);
#endif // FSC_DEBUG
...
void StateMachineIllegalCable(void)
{
    ...
    {
        CCTermPrevious = DecodeCCTermination();
        WriteStateLog(&TypeCStateLog, 100, 0, 0);
    }
